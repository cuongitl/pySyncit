...
log_file = f"{log_dir}/pySyncit_{current_datetime}.log"
...
def copy_file_or_dir(_src, _dest, _update=False, _exclude=None):
	...
	if os.path.isfile(_src):
		logging.info("Case: src is a file")
		...
		if not os.path.isfile(dest_file):
			if _exclude:
				...
				if file_ext in _exclude:
					return
			if _update and not is_modified(_src, dest_file):
				return
	...
	# If src is a directory (folder), copy the entire directory
		...
		logging.info("Case: src is a directory")
		...
		for file in os.listdir(_src):
			...
			# logging.info(f"full_file_path: {full_file_path}")
			if os.path.isfile(full_file_path):
				...
				if _update and not is_modified(full_file_path,dest_file):
					# logging.info(f"update: {_update} / - {is_modified(full_file_path, dest_file)}")
					continue
				logging.info(f"Copied: {full_file_path} to {new_dest_dir}")
	...
	# If src is a pattern (e.g., *.txt), copy matching files
		...
		logging.info("Case: copy matching files")
		...
		for file in matching_files:
			...
			if _exclude:
				...
				if file_ext in _exclude:
					continue
			if _update and not is_modified(file, dest_file):
				continue
...
def check_file_modified(_src_file_path, _file_json):
	...
	if current_attributes['size'] != previous_attributes['size'] or current_attributes['modified_time'] != previous_attributes['modified_time']:
		...
		# print(f"{_src_file_path} - File has been modified.")
		logging.info(f"{_src_file_path} - File has been modified.")
		# Update the JSON file with the new attributes
		write_attributes_to_json(_src_file_path, _file_json)
		return True
	else:
		# print(f"{_src_file_path} - File has not been modified.")
		return False


def is_modified(_src_file_path, _dest_file_path):
	# Extract the filename
	file_name = os.path.basename(_src_file_path)
	# print(file_name)
	# without extension
	...
	is_m = check_file_modified(_src_file_path, file_json)
	if not is_m:
		# Check file is exist on dest_dir or not?
